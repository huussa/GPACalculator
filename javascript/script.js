function addSubject(){
    let counter = document.querySelectorAll('.subject').length + 1;

    const subjectsContainer = document.querySelector('.subjects-container');
    const newSubject = document.createElement('div');
    newSubject.classList.add('subject');
    newSubject.innerHTML = `            
            <p>subject <span class="number-of-subjects">${counter}</span></p>
            <div class="input-group subject-name">
                <h3>subject name</h3>
                <input type="text" name="subject-name" placeholder="Enter subject name">
            </div>
...
function showResult(){
    ...
    if (totalHours === 0 || isNaN(gpa)) {
        document.getElementById("gpa").textContent = "don't forget to add hours and grades";
        return;
    }
    if(system == 1){
        gpa += 1;
